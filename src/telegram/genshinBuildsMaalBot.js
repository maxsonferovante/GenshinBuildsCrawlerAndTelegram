...
const LINK_REPOSITORY =
  "https://github.com/maxsonferovante/GenshinBuildsCrawlerAndTelegram";
...
export default class GenshinBuildsMaalBot {
      ...
      cron.schedule("20 9 * * *", async () => {
        ...
        for (const user of usersSaved) {
          ...
          try {
            ...
            await this.bot.sendMessage(
              chatId,
              `Armas Disponível para Farmar hoje (${DiaDaSemana.obterDataAtualComDiaDaSemana()}) são : \n\n`,
              { parse_mode: "HTML" }
            );
            ...
            for (const key in this.crawler.dictWeapon) {
              ...
              await this.bot.sendMessage(
                chatId,
                `<b>${key}</b> \n\n${this.crawler.dictWeapon[key]
                  .map((/** @type {{ url: any; name: any; }} */ weapon) => {
                    quantityWeapons--;
                    const textResponse = `${
                      this.crawler.dictWeapon[key].length - quantityWeapons + 1
                    } - <a href="${weapon.url}">${weapon.name}</a>`;
                    return textResponse;
                  })
                  .join(" \n")}`,
                { parse_mode: "HTML" }
          ...
          try {
            ...
            await this.bot.sendMessage(
              chatId,
              `Personagens Disponível para Farmar hoje (${DiaDaSemana.obterDataAtualComDiaDaSemana()}) são : \n\n`,
              { parse_mode: "HTML" }
            );

            for (const key in this.crawler.dictCharacter) {
              let quantityCharacters =
                this.crawler.dictCharacter[key].length + 1;
              await this.bot.sendMessage(
                chatId,
                `<b>${key}</b> \n\n${this.crawler.dictCharacter[key]
                  .map((/** @type {{ url: any; name: any; }} */ character) => {
                    quantityCharacters--;
                    const textResponse = `${
                      this.crawler.dictCharacter[key].length -
                      quantityCharacters +
                      1
        ...
        console.log(
          `Finalizando o serviço às ${dataFinalizacao.getHours()}:${dataFinalizacao.getMinutes()}`
        );
      });
    } catch (error) {
      console.log(error);
      await this.bot.sendMessage(
        process.env.CHAT_ID_ADM,
        "Ocorreu um erro ao tentar enviar o SendUpdateDaily."
      );
  ...
  async welconeGenshinBuidlsMaalBot() {
    this.bot.onText(/\/start/, async (msg) => {
      try {
        ...
        const userExist = await this.postgreUserRepository.existsByChatId(
          chatId
        );
        let capition = "";
        if (userExist) {
        ...
        await this.bot.sendMessage(
          chatId,
          "Possivelmente estamos com problemas, tente novamente mais tarde."
        );
        console.error(error);
      }
    });
    ...
    this.bot.onText(/\/characters/, async (msg) => {
      ...
      try {
        ...
        this.bot.sendMessage(
          chatId,
          "Ocorreu um erro ao tentar obter os personagens."
        );
        console.error(error);
      } finally {
        await this.sendInfoDeveloper({
  ...
  async sendWeapons() {
    this.bot.onText(/\/weapons/, async (msg) => {
      ...
      try {
        ...
        this.bot.sendMessage(
          chatId,
          "Ocorreu um erro ao tentar obter as armas."
        );
        console.error(error);
      } finally {
        await this.sendInfoDeveloper({
  ...
  async sendInfoDeveloper(data) {
    ...
    const mensagem = data.userExist
      ? `O usuário usou o comando: ${data.command}`
      : `Um novo usuário acessou o bot Genshin-Builds Maal-Bot.
